...
exports.getNotifFollow = async (id_user) => {
  const query =
    `SELECT u.username, uf.id_user, CONCAT("${process.env.PROFILE_PATH}", u.profile_picture) as profile_picture, uf.followed_at FROM user_follow uf JOIN users u ON u.id_user = uf.id_user WHERE uf.id_following = ?`
  const data = await dbPool.query(query, [id_user])
